...
class Ocean {
 public:
  ...
  void SimulateOneStep()throw(int);
  ...
  void PopulationCensus(int& nshark, int& nfish);
   //my helper functions and thread vector
   std::vector<std::thread> ThreadVector;
   int seaExist(int x, int y);
   int sumfish(int startx, int endx);
   int sumshark(int startx, int endx);
   bool checSize();
 private:
